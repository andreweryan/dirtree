...
def get_sorted_items(path: Path) -> Tuple[List[Path], List[Path]]:
    ...
    dirs = sorted(
        [item for item in items if item.is_dir()], key=lambda x: x.name.lower()
    )
    files = sorted(
...
def print_tree(
    ...
    for dir_path in dirs:
        ...
        print_tree(
            dir_path, new_prefix, is_last, max_depth, current_depth + 1, show_hidden
        )
...
Examples:
  ...
  %(prog)s /path/to/dir -a     # Show hidden files and directories
    ...
    parser.add_argument("path", help="Directory path to display tree for")
    parser.add_argument(
        "-d", "--max-depth", type=int, help="Maximum depth of directory tree to display"
    )
    parser.add_argument(
